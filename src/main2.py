...
"background-color: #FFFFFF;")
        ...
        self.layoutWidget.setGeometry(QtCore.QRect(10, 40, 400, 800))
...
" box-sizing: border-box;")
    ...
    ###buttons functions###
        self.startButton.clicked.connect(self.resetAction)
    ...
    ######GroupBox Scrolling ###################"
        ...
        for n in range(100):
            formLayout.addRow(self.Datelabel)
        ...
        ### stop thread et affichage des popups ####
    def stopThread(self):
        msg = QMessageBox()
        
        if hasattr(self, "th") :
        
            if not self.th.stopped :
                for sensor in self.sensorsIndicators :
                    for label in self.sensorsIndicators[sensor]:
                        label.deleteLater()
                self.th.stop()
                msg.setText("thread stopped")
                msg.setIcon(QMessageBox.Information)
                x = msg.exec_()
                
            else :
                msg.setText("Start first")
        ...
        ### Startbutton action && affichage des Sensor Values ###
  
    def resetAction(self):
        
        selectedCom  = self.COM.currentText()
        self.th = Thread()
        self.th.port = selectedCom 
        self.th.changeLabelText.connect(self.setLabelText)
        self.th.start()
...
#
        ### changer la valeur de label en SensorValue ###
    def setLabelText(self, text):
        now = datetime.now()
        sensor , value = text.split(":")
        date_time = now.strftime("%m/%d/%Y, %H:%M:%S")
        if sensor in self.sensorsIndicators:
            list_labels = self.sensorsIndicators[sensor]
            list_labels[1].setText(value)
            
            list_labels[2].setText(date_time)
        else :
            list_labels = self.addDisplayLables()
            list_labels[0].setText(sensor)
            list_labels[1].setText(value)
            list_labels[2].setText(date_time)
            self.sensorsIndicators[sensor] = list_labels
        
      ### initialising COM###  
    def initialzeCom(self):
        listComs = SerialCom.findCom()
        for com in listComs :
            
                self.COM.addItem(com)
